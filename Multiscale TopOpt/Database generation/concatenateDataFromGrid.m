...
%     locfilename3='struGridNoNoisetot.txt';
    ...
    %for i=1:2^12
    for i=1+256*(k-1):min(256*k,2^12)%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %for i=1:size(LHpoints,2)
    %for i=14500:14500
        pointfilename=['InitialDB/point',num2str(i)];
        pointfileID=fopen(pointfilename);
        pointdata = fscanf(pointfileID,'%f',[27 inf]); %matrix containing at the i-th column the the values of Q of the i-th optimised cell 
        
        pointobjfilename=['InitialDB/objpoint',num2str(i)];
        pointobjfileID=fopen(pointobjfilename);
        pointobjdata = fscanf(pointobjfileID,'%f',[30 inf]);
        
        pointstrufilename=['InitialDB/strupoint',num2str(i)];
