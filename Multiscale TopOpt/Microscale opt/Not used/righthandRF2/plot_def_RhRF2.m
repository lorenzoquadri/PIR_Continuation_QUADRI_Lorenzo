function plot_def_RhRF2(U,nelx,nely,nelz)
if nargin==4
    %
%     nelz=0;
    % 
...
%     z_fin(:)=z_in(:);
    ...
    plot3(z_fin(:),x_fin(:),y_fin(:),'ro');
    hold on
    plot3(z_fin(1),x_fin(1),y_fin(1),'r*',z_fin(nely+1),x_fin(nely+1),y_fin(nely+1),'r*',z_fin((nely+1)*nelx+1),x_fin((nely+1)*nelx+1),y_fin((nely+1)*nelx+1),'r*',z_fin((nely+1)*(nelx+1)),x_fin((nely+1)*(nelx+1)),y_fin((nely+1)*(nelx+1)),'r*',...
        z_fin((nely+1)*(nelx+1)*nelz+1),x_fin((nely+1)*(nelx+1)*nelz+1),y_fin((nely+1)*(nelx+1)*nelz+1),'r*',z_fin((nely+1)*(nelx+1)*nelz+nely+1),x_fin((nely+1)*(nelx+1)*nelz+nely+1),y_fin((nely+1)*(nelx+1)*nelz+nely+1),'r*',z_fin((nely+1)*(nelx+1)*nelz+(nely+1)*nelx+1),x_fin((nely+1)*(nelx+1)*nelz+(nely+1)*nelx+1),y_fin((nely+1)*(nelx+1)*nelz+(nely+1)*nelx+1),'r*',z_fin((nely+1)*(nelx+1)*(nelz+1)),x_fin((nely+1)*(nelx+1)*(nelz+1)),y_fin((nely+1)*(nelx+1)*(nelz+1)),'r*');
    
    plot3(z_in(:),x_in(:),y_in(:),'bo');
    plot3(z_in(1),x_in(1),y_in(1),'b*',z_in(nely+1),x_in(nely+1),y_in(nely+1),'b*',z_in((nely+1)*nelx+1),x_in((nely+1)*nelx+1),y_in((nely+1)*nelx+1),'b*',z_in((nely+1)*(nelx+1)),x_in((nely+1)*(nelx+1)),y_in((nely+1)*(nelx+1)),'b*',...
        z_in((nely+1)*(nelx+1)*nelz+1),x_in((nely+1)*(nelx+1)*nelz+1),y_in((nely+1)*(nelx+1)*nelz+1),'b*',z_in((nely+1)*(nelx+1)*nelz+nely+1),x_in((nely+1)*(nelx+1)*nelz+nely+1),y_in((nely+1)*(nelx+1)*nelz+nely+1),'b*',z_in((nely+1)*(nelx+1)*nelz+(nely+1)*nelx+1),x_in((nely+1)*(nelx+1)*nelz+(nely+1)*nelx+1),y_in((nely+1)*(nelx+1)*nelz+(nely+1)*nelx+1),'b*',z_in((nely+1)*(nelx+1)*(nelz+1)),x_in((nely+1)*(nelx+1)*(nelz+1)),y_in((nely+1)*(nelx+1)*(nelz+1)),'b*');
