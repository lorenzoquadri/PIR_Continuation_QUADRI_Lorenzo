...
%     n3_with_n1 = [reshape(permute(nodenrs(1:nfarthestUsedx0y0z0,1,1:nfarthestUsedz0x0y0),[3,1,2]),[],1);reshape(permute(nodenrs(nfarthestUsedx0yMz0:nely+1,1,1:nfarthestUsedz0x0y0),[3,1,2]),[],1);reshape(permute(nodenrs(nfarthestUsedx0yMz0:nely+1,1,nfarthestUsedzMx0y0:nelz+1),[3,1,2]),[],1);reshape(permute(nodenrs(1:nfarthestUsedx0y0z0,1,nfarthestUsedzMx0y0:nelz+1),[3,1,2]),[],1);... %left
%         reshape(permute(nodenrs(nely+1,1:nfarthestUsedy0x0z0,1:nfarthestUsedz0x0y0),[2,3,1]),[],1);reshape(permute(nodenrs(nely+1,nfarthestUsedy0xMz0:nelx+1,1:nfarthestUsedz0x0y0),[2,3,1]),[],1);reshape(permute(nodenrs(nely+1,nfarthestUsedy0xMz0:nelx+1,nfarthestUsedzMx0y0:nelz+1),[2,3,1]),[],1);reshape(permute(nodenrs(nely+1,1:nfarthestUsedy0x0z0,nfarthestUsedzMx0y0:nelz+1),[2,3,1]),[],1);... %bottom
%         reshape(nodenrs(1:nfarthestUsedx0y0z0,1:nfarthestUsedy0x0z0,1),[],1);reshape(nodenrs(nfarthestUsedx0yMz0:nely+1,1:nfarthestUsedy0x0z0,1),[],1);reshape(nodenrs(nfarthestUsedx0yMz0:nely+1,nfarthestUsedy0xMz0:nelx+1,1),[],1);reshape(nodenrs(1:nfarthestUsedx0y0z0,nfarthestUsedy0xMz0:nelx+1,1),[],1)]'; %front
%     idx3 = ~ismember(n3_with_n1',n1','rows');
%     n3 = n3_with_n1(idx3);
%     d3 = reshape([(3*n3-2);(3*n3-1);3*n3],1,3*size(n3,2)); 
%     n4_with_n1 = [reshape(permute(nodenrs(1:nfarthestUsedx0y0z0,nelx+1,1:nfarthestUsedz0x0y0),[3,1,2]),[],1);reshape(permute(nodenrs(nfarthestUsedx0yMz0:nely+1,nelx+1,1:nfarthestUsedz0x0y0),[3,1,2]),[],1);reshape(permute(nodenrs(nfarthestUsedx0yMz0:nely+1,nelx+1,nfarthestUsedzMx0y0:nelz+1),[3,1,2]),[],1);reshape(permute(nodenrs(1:nfarthestUsedx0y0z0,nelx+1,nfarthestUsedzMx0y0:nelz+1),[3,1,2]),[],1);... % right
%         reshape(permute(nodenrs(1,1:nfarthestUsedy0x0z0,1:nfarthestUsedz0x0y0),[2,3,1]),[],1);reshape(permute(nodenrs(1,nfarthestUsedy0xMz0:nelx+1,1:nfarthestUsedz0x0y0),[2,3,1]),[],1);reshape(permute(nodenrs(1,nfarthestUsedy0xMz0:nelx+1,nfarthestUsedzMx0y0:nelz+1),[2,3,1]),[],1);reshape(permute(nodenrs(1,1:nfarthestUsedy0x0z0,nfarthestUsedzMx0y0:nelz+1),[2,3,1]),[],1);... %top
%         reshape(nodenrs(1:nfarthestUsedx0y0z0,1:nfarthestUsedy0x0z0,nelz+1),[],1);reshape(nodenrs(nfarthestUsedx0yMz0:nely+1,1:nfarthestUsedy0x0z0,nelz+1),[],1);reshape(nodenrs(nfarthestUsedx0yMz0:nely+1,nfarthestUsedy0xMz0:nelx+1,nelz+1),[],1);reshape(nodenrs(1:nfarthestUsedx0y0z0,nfarthestUsedy0xMz0:nelx+1,nelz+1),[],1)]'; %rear
%     idx4 = ~ismember(n4_with_n1',n1','rows');
%     n4 = n4_with_n1(idx4);
%     d4 = reshape([(3*n4-2);(3*n4-1);3*n4],1,3*size(n4,2)); 
    ...
    % wfixed = [repmat(ufixed(4:6,:),((nfarthestUsedx0y0z0)+(nely+2-nfarthestUsedx0yMz0))*((nfarthestUsedz0x0y0)+(nelz+2-nfarthestUsedzMx0y0))-4,1); repmat(ufixed(10:12,:),((nfarthestUsedy0x0z0)+(nelx+2-nfarthestUsedy0xMz0))*((nfarthestUsedz0x0y0)+(nelz+2-nfarthestUsedzMx0y0))-4,1); repmat(ufixed(13:15,:),((nfarthestUsedx0y0z0)+(nely+2-nfarthestUsedx0yMz0))*((nfarthestUsedy0x0z0)+(nelx+2-nfarthestUsedy0xMz0))-4,1)];
